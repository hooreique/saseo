...
def main [] {
    ...
    if ($result.exit_code | into string) != "7" {
        print "not ok - duplicate marker exit code"
        print "expected:"
        print "7"
        print "actual:"
    ...
    print "ok - duplicate marker exit code"
    assert_error_output_format $result.stderr "duplicate marker"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join input.txt))
    if $actual != $expected {
        print "not ok - duplicate marker keeps target"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - duplicate marker keeps target"
    rm --force $work
}
