...
def main [] {
    ...
    if ($result.exit_code | into string) != "0" {
        print "not ok - remove absent managed block ignores stdin exit code"
        print "expected:"
        print "0"
        print "actual:"
    ...
    print "ok - remove absent managed block ignores stdin exit code"
    if $result.stdout != "" {
        print "not ok - remove absent managed block ignores stdin stdout"
        print "expected:"
        print ""
        print "actual:"
        print $result.stdout
    ...
    print "ok - remove absent managed block ignores stdin stdout"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join expected.txt))
    if $actual != $expected {
        print "not ok - remove absent managed block"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - remove absent managed block"
    let after_meta = (ls -l $work | get 0 | select inode modified size)
    if $after_meta != $before_meta {
        print "not ok - remove absent managed block leaves target untouched"
        print "expected:"
        print $before_meta
        print "actual:"
        print $after_meta
    ...
    print "ok - remove absent managed block leaves target untouched"
    rm --force $work
}
