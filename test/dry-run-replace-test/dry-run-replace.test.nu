...
def main [] {
    ...
    let result = do {
        "export X=baz\nexport Z=qux\n" | ^nu $saseo --dry-run --marker SASEO $work
    } | complete
    if ($result.exit_code | into string) != "0" {
        print "not ok - dry-run replace managed block exit code"
        print "expected:"
        print "0"
        print "actual:"
    ...
    print "ok - dry-run replace managed block exit code"
    if $result.stdout != "3 4\n" {
        print "not ok - dry-run replace managed block stdout"
        print "expected:"
        print "3 4\n"
        print "actual:"
        print $result.stdout
    ...
    print "ok - dry-run replace managed block stdout"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join input.txt))
    if $actual != $expected {
        print "not ok - dry-run replace leaves target contents untouched"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - dry-run replace leaves target contents untouched"
    let after_meta = (ls -l $work | get 0 | select inode modified size)
    if $after_meta != $before_meta {
        print "not ok - dry-run replace leaves target metadata untouched"
        print "expected:"
        print $before_meta
        print "actual:"
        print $after_meta
    ...
    print "ok - dry-run replace leaves target metadata untouched"
    rm --force $work
}
