# Spec: SASEO-ADD-REPLACE, SASEO-EXIT-CODES, SASEO-ERROR-OUTPUT
...
def main [] {
    ...
    let result = do { ^nu $saseo $work } | complete
    if ($result.exit_code | into string) != "4" {
        print "not ok - empty stdin add exit code"
        print "expected:"
        print "4"
        print "actual:"
    ...
    print "ok - empty stdin add exit code"
    assert_error_output_format $result.stderr "empty stdin add"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join input.txt))
    if $actual != $expected {
        print "not ok - empty stdin add keeps target"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - empty stdin add keeps target"
    rm --force $work
}
