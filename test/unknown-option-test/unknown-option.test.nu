...
def main [] {
    ...
    let result = do { ^nu $saseo --bogus $work } | complete
    if ($result.exit_code | into string) != "3" {
        print "not ok - unknown option exit code"
        print "expected:"
        print "3"
        print "actual:"
    ...
    print "ok - unknown option exit code"
    assert_error_output_format $result.stderr "unknown option"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join input.txt))
    if $actual != $expected {
        print "not ok - unknown option keeps target"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - unknown option keeps target"
    rm --force $work
}
