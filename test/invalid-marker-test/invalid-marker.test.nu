...
def main [] {
    ...
    let result = do {
        "export X=foo\nexport Y=bar\n" | ^nu $saseo --marker "BAD/MARKER" $work
    } | complete
    if ($result.exit_code | into string) != "6" {
        print "not ok - invalid marker exit code"
        print "expected:"
        print "6"
        print "actual:"
    ...
    print "ok - invalid marker exit code"
    assert_error_output_format $result.stderr "invalid marker"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join input.txt))
    if $actual != $expected {
        print "not ok - invalid marker keeps target"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - invalid marker keeps target"
    rm --force $work
}
