...
def main [] {
    ...
    let result = do { ^nu $saseo --marker } | complete
    if ($result.exit_code | into string) != "3" {
        print "not ok - missing marker value exit code"
        print "expected:"
        print "3"
        print "actual:"
    ...
    print "ok - missing marker value exit code"
    assert_error_output_format $result.stderr "missing marker value"
}
