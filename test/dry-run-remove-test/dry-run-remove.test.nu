# Spec: SASEO-DRY-RUN, SASEO-REMOVE, SASEO-AFFECTED-LINES
...
def main [] {
    ...
    if ($result.exit_code | into string) != "0" {
        print "not ok - dry-run remove managed block ignores stdin exit code"
        print "expected:"
        print "0"
        print "actual:"
    ...
    print "ok - dry-run remove managed block ignores stdin exit code"
    if $result.stdout != "3\n" {
        print "not ok - dry-run remove managed block stdout"
        print "expected:"
        print "3\n"
        print "actual:"
        print $result.stdout
    ...
    print "ok - dry-run remove managed block stdout"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join input.txt))
    if $actual != $expected {
        print "not ok - dry-run remove leaves target contents untouched"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - dry-run remove leaves target contents untouched"
    let after_meta = (ls -l $work | get 0 | select inode modified size)
    if $after_meta != $before_meta {
        print "not ok - dry-run remove leaves target metadata untouched"
        print "expected:"
        print $before_meta
        print "actual:"
        print $after_meta
    ...
    print "ok - dry-run remove leaves target metadata untouched"
    rm --force $work
}
