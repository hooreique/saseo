...
def main [] {
    ...
    if ($result.exit_code | into string) != "7" {
        print "not ok - start only exit code"
        print "expected:"
        print "7"
        print "actual:"
    ...
    print "ok - start only exit code"
    assert_error_output_format $result.stderr "start only"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join input.txt))
    if $actual != $expected {
        print "not ok - start only keeps target"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - start only keeps target"
    rm --force $work
}
