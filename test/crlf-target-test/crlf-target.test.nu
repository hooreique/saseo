...
def main [] {
    ...
    if ($result.exit_code | into string) != "8" {
        print "not ok - crlf target exit code"
        print "expected:"
        print "8"
        print "actual:"
    ...
    print "ok - crlf target exit code"
    assert_error_output_format $result.stderr "crlf target"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join input.txt))
    if $actual != $expected {
        print "not ok - crlf target keeps target"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - crlf target keeps target"
    rm --force $work
}
