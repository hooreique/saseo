...
def main [] {
    ...
    if ($result.exit_code | into string) != "7" {
        print "not ok - reversed marker exit code"
        print "expected:"
        print "7"
        print "actual:"
    ...
    print "ok - reversed marker exit code"
    assert_error_output_format $result.stderr "reversed marker"
    let actual = (open --raw $work)
    let expected = (open --raw ($test_dir | path join input.txt))
    if $actual != $expected {
        print "not ok - reversed marker keeps target"
        print "expected:"
        print $expected
        print "actual:"
        print $actual
    ...
    print "ok - reversed marker keeps target"
    rm --force $work
}
